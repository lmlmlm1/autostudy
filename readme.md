...
### 1. 강의록 준비
- **실행 위치**: 로컬 `WATCH_PATH/lecture/`
- **입력**: 줄필기·야붙 PDF 또는 이미 병합된 PDF
- **핵심 동작**: 파일명 정리와 PDF 병합
- **산출물**: `WATCH_PATH/{base_name}.pdf`

### 2. 1차 처리
- **실행 위치**: 로컬
- **입력**: 원본 PDF
- **핵심 동작**: 페이지별 텍스트 추출, OCR 보완, Whisper 초기 프롬프트 생성
- **산출물**: `_강의자료.txt`, `_whisperkeyword.txt`

### 3. 음성 전사
- **실행 위치**: Google Colab
- **입력**: `.m4a` 또는 변환 가능한 `.mp4`, 키워드 파일
- **핵심 동작**: Whisper `medium` 모델로 한국어 전사
- **산출물**: `_음성스크립트.txt`

### 4. 2차 처리
- **실행 위치**: 로컬
- **입력**: 원본 PDF·음성 파일과 두 텍스트 파일
- **핵심 동작**: 스크립트 교정, 요약, 결과 폴더 이동
- **산출물**: 교정본, JSON, 강의별 폴더

### 5. 연동·출력
- **실행 위치**: 로컬 및 연동 서비스
- **입력**: 처리 완료 강의 폴더
- **핵심 동작**: Notion 업로드, Anki 생성, 복습용 PDF 생성
- **산출물**: Notion 페이지, CSV·APKG, `_scripted.pdf`

### 6. 날짜별 합본
- **실행 위치**: 로컬 수동 실행
- **입력**: 여러 강의의 `_scripted.pdf`
- **핵심 동작**: 같은 날짜 강의의 복습용 PDF 병합
- **산출물**: `merged/{날짜}_merged_scripted.pdf`
